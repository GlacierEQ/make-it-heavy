...
class ClaimAwareAdaptiveWorkerLoop(AdaptiveWorkerLoop):
    ...
    def parse_evidence_registry(mission: str) -> EvidenceRegistry:
        """Parse and validate a bounded immutable evidence registry embedded in a mission."""
    ...
    def evaluate_claim_discipline(
        ...
        source_score = 1.0
        if observed_sources:
            valid_source_count = len(observed_sources) - sum(
                1
                for reference in invalid_observed_references
                if "#" not in reference or not registry_active
            )
        ...
        pointer_gate_pass = not registry_active or (
            not invalid_observed_references
            and len(resolved_pointers) == len(observed_sources)
        )
        passed = (
